...
use kumandra_primitives::{AccountId, Balance};
	...
	fn build(&self, nonce: u32) -> std::result::Result<OpaqueExtrinsic, &'static str> {
		with_client! {
			self.client.as_ref(), client, {
				...
				let period = kumandra_runtime::BlockHashCount::get().checked_next_power_of_two().map(|c| c / 2).unwrap_or(2) as u64;
...
impl BenchmarkCallSigner<kumandra_runtime::RuntimeCall, sp_core::sr25519::Pair>
	for FullClient<polkadot_runtime::RuntimeApi, KumandraExecutorDispatch>
{
	fn sign_call(
		&self,
		call: kumandra_runtime::RuntimeCall,
		nonce: u32,
		current_block: u64,
		period: u64,
		genesis: H256,
		...
		use kumandra_runtime as runtime;
		...
		let extra: runtime::SignedExtra = (
			...
			kumandra_runtime::claims::PrevalidateAttests::<runtime::Runtime>::new(),
		);
		...
		runtime::UncheckedExtrinsic::new_signed(
			...
			kumandra_primitives::Signature::Sr25519(signature.clone()),
			extra,
		)
		.into()
	}
