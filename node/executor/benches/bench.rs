...
use kumanddra_executor::ExecutorDispatch;
use kumanddra_primitives::{BlockNumber, Hash};
use kumanddra_testing::keyring::*;
...
pub fn compact_code_unwrap() -> &'static [u8] {
	kumandra_runtime::WASM_BINARY.expect(
...
const TRANSACTION_VERSION: u32 = kumandra_runtime::VERSION.transaction_version;

const SPEC_VERSION: u32 = kumandra_runtime::VERSION.spec_version;
...
fn sign(xt: CheckedExtrinsic) -> UncheckedExtrinsic {
	kumanddra_testing::keyring::sign(xt, SPEC_VERSION, TRANSACTION_VERSION, GENESIS_HASH)
}
...
fn bench_execute_block(c: &mut Criterion) {
	...
	for strategy in execution_methods {
		group.bench_function(format!("{:?}", strategy), |b| {
			let genesis_config = kumanddra_testing::genesis::config(Some(compact_code_unwrap()));
