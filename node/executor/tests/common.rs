...
pub const SPEC_VERSION: u32 = kumandra_runtime::VERSION.spec_version;

pub const TRANSACTION_VERSION: u32 = kumandra_runtime::VERSION.transaction_version;

pub type TestExternalities<H> = CoreTestExternalities<H>;

pub fn sign(xt: CheckedExtrinsic) -> UncheckedExtrinsic {
	kumandra_testing::keyring::sign(xt, SPEC_VERSION, TRANSACTION_VERSION, GENESIS_HASH)
}
...
pub fn new_test_ext(code: &[u8]) -> TestExternalities<BlakeTwo256> {
	let ext = TestExternalities::new_with_code(
		code,
		kumandra_testing::genesis::config(Some(code)).build_storage().unwrap(),
	);
	ext
}
