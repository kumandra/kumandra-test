...
use futures::prelude::*;
use kumandra_executor::ExecutorDispatch;
use kumandra_primitives::Block;
...
pub fn create_extrinsic(
	...
	function: impl Into<kumandra_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> kumandra_runtime::UncheckedExtrinsic {
	...
	let period = kumandra_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let extra: kumandra_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<kumandra_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<kumandra_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<kumandra_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<kumandra_runtime::Runtime>::new(),
		frame_system::CheckEra::<kumandra_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<kumandra_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<kumandra_runtime::Runtime>::new(),
		// pallet_asset_tx_payment::ChargeAssetTxPayment::<kumandra_runtime::Runtime>::from(tip),
		pallet_transaction_payment::ChargeTransactionPayment::<kumandra_runtime::Runtime>::from(0),
	);

	let raw_payload = kumandra_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			kumandra_runtime::VERSION.spec_version,
			kumandra_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	kumandra_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		kumandra_runtime::Signature::Sr25519(signature),
		extra,
	)
}
...
> {
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
			...
			kumandra_rpc::create_full(deps, rpc_backend.clone()).map_err(Into::into)
		};
...
mod tests {
	...
	use codec::Encode;
	use kumandra_primitives::{Block, DigestItem, Signature};
